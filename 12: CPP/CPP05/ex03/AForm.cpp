...
int			AForm::getG_toSign() const {
	return _toSign;
}

int			AForm::getG_toExec() const {
	return _toExec;
}
...
std::ostream& operator<<(std::ostream& os, const AForm& f) {
	os << "| Name : " << f.getName() << " | Signed is : " << f.getSigned() << " | Grade to Sign : " << f.getG_toSign() << " | Grade to Exec : " << f.getG_toExec() << std::endl;
	return os;
}